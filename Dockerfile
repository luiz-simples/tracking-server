FROM debian:jessie
MAINTAINER Luiz <https://github.com/luiz-simples>
...
#NodeJS v4.3.1 - Long Term Support Release
...
RUN echo "%sudo ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers && \
    useradd -u 5001 -G users,sudo -d /server --shell /bin/bash -m server && \
    echo "secret\nsecret" | passwd server

RUN usermod -u 1000 server
